# -*- coding: utf-8 -*-
"""
Created on Wed Aug 28 15:05:12 2024
...
param_ranges = {
    'S0': (10, 500),      # Underlying asset price
    'K': (10, 500),       # Strike price
    'T': (0.1, 5),        # Time to expiry in years
...
# Implement the greek Calculations
def calculate_gamma(S0, K, T, sigma, r):
    d1 = (np.log(S0 / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
...
def calculate_vega(S0, K, T, sigma, r):
    d1 = (np.log(S0 / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    vega = S0 * norm.pdf(d1) * np.sqrt(T)
    return vega


def calculate_rho(S0, K, T, sigma, r):
    d2 = (np.log(S0 / K) + (r - 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    rho = K * T * np.exp(-r * T) * norm.cdf(d2)
    return rho

def calculate_delta(S0, K, T, sigma, r):
    d1 = (np.log(S0 / K) + (r + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
    delta = norm.cdf(d1)
    return delta
...
#calculate gamma, vega, rho values for black scholes
gamma_values = np.array([calculate_gamma(*params) for params in param_grid])
vega_values = np.array([calculate_vega(*params) for params in param_grid])
rho_values = np.array([calculate_rho(*params) for params in param_grid])
delta_values = np.array([calculate_delta(*params) for params in param_grid])
...
data_BS_greeks_HW = pd.DataFrame({
    'T': T,
    'Moneyness': moneyness,
...
print(data_BS_greeks_HW.describe())
print("\nChecking correlations...")
print(data_BS_greeks_HW.corr())



# Split the dataset into inputs and targets
X_BS_greeks_HW= data_BS_greeks_HW[['T', 'Moneyness', 'Gamma', 'Vega', 'Rho', 'Delta', 'BS_price']]
y_BS_greeks_HW = data_BS_greeks_HW['HW_price']

# Split into training and test sets
X_train, X_test, y_BS_greeks_HW_train, y_BS_greeks_HW_test = train_test_split(X_BS_greeks_HW, y_BS_greeks_HW, test_size=0.2, random_state=42)
...
# Fit the model
mlp.fit(X_train_scaled, y_BS_greeks_HW_train)


# Predict and evaluate
y_BS_greeks_HW_pred = mlp.predict(X_test_scaled)
print(f'Hull-White  Model MSE: {mean_squared_error(y_BS_greeks_HW_test, y_BS_greeks_HW_pred)}')

# Save the model and scaler
joblib.dump(mlp, 'BS_greeks_HW_best_model.pkl')
...
plt.figure(figsize=(12, 6))
plt.scatter(y_BS_greeks_HW_test, y_BS_greeks_HW_pred, alpha=0.5, label='Hull-White Predictions')
plt.plot([min(y_BS_greeks_HW_test), max(y_BS_greeks_HW_test)], [min(y_BS_greeks_HW_test), max(y_BS_greeks_HW_test)], 'r', linestyle='--')
...
errors_BS_greeks_HW = y_BS_greeks_HW_test - y_BS_greeks_HW_pred

plt.figure(figsize=(12, 6))
sns.histplot(errors_BS_greeks_HW, bins=150, kde=True, color='blue', label='Hull-White Errors')
