...
HW Model trains a NN with the Hull-White model using time to expiry (T) and moneyness (S/K) - The Hull-White Volatility model is used to price european call options via fourier transform. 
...
calibrated NN  uses real data to calibrate part of the model (by using real data then kernel density estimation to generate synthetic data), then trains the network using the greeks and T and S/K. The zipped file contains the excel data used in the calibration, so you will need to download it, then rewrite the line of code that reads the data to wherever you have saved it.
